...
### Added
- **Receipts say which engine spoke.** The approved-version receipt, its
  `manifest.json`, and the hosted verify page now name the TTS engine —
  Chatterbox, Chatterbox Turbo, or Qwen3 TTS — that rendered each chunk,
  alongside its voice. The seal panel gains an Engine line summarizing every
  engine heard in the final, and the archive's per-clip listing names one for
  every saved take. The engine is frozen onto each take the moment it renders,
  so re-pointing a voice at a different engine later cannot rewrite what an
  existing receipt reports.
