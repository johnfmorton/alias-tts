...
Genblaze provider connectors for the [Bespoken](https://github.com/johnfmorton) self-hosted,
...
|---|---|---|
| `BespokenTTSProvider` | ✅ Posture A | `POST /v1/text-to-speech/{voice_id}` (full pipeline) |
| `BespokenQAProvider` | ⏳ Posture B | `POST /v1/internal/score` (ASR quality verdict) |
| `BespokenStitchProvider` | ⏳ Posture B | `POST /v1/internal/stitch` (concat chunks) |

`BespokenTTSProvider` is a `SyncProvider`: it calls Bespoken, buffers the
...
| Var | Purpose |
|---|---|
| `BESPOKEN_BASE_URL` | Base URL of the Bespoken service (e.g. `https://tts.example.com`) |
| `BESPOKEN_API_KEY` | API key sent as the `xi-api-key` header |
...
from genblaze_bespoken import BespokenTTSProvider

result = (
    Pipeline("narration")
    .step(BespokenTTSProvider(), model="default",
