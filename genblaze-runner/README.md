...
The Genblaze-owned orchestrator for the Alias TTS pipeline. Drives
...
```bash
pip install -e ../connectors/genblaze-alias   # sibling connector (not on PyPI)
pip install -e ".[dev]"
```
...
| Var | Purpose |
|---|---|
| `ALIAS_BASE_URL` | Base URL of the running Alias service |
| `ALIAS_INTERNAL_SECRET` | Shared secret for `/v1/internal/*` (`X-Internal-Secret`) |
| `B2_BUCKET` / `B2_KEY_ID` / `B2_APP_KEY` / `B2_REGION` | Backblaze B2 provenance store (omit `B2_BUCKET` to run sink-less) |
| `TTS_STORAGE_ROOT` | Optional shared-bucket subfolder — uploads go under `<root>/genblaze/` to match the app (read directly from the app's env name; `ALIAS_STORAGE_ROOT` overrides) |
...
# Real proof-of-life (B2 + Alias env set) — writes takes + manifests to B2:
