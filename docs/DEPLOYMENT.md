...
```
`APP_KEY` is generated by Forge during install. All three catalog models ship
as config defaults with pinned versions (classic `resemble-ai/chatterbox`,
`resemble-ai/chatterbox-turbo`, and `qwen/qwen3-tts`), so you only need the
token. To override the defaults:
`REPLICATE_CHATTERBOX_MODEL`/`REPLICATE_CHATTERBOX_VERSION` (classic),
`REPLICATE_CHATTERBOX_TURBO_MODEL`/`REPLICATE_CHATTERBOX_TURBO_VERSION` (turbo),
`REPLICATE_QWEN3_TTS_MODEL`/`REPLICATE_QWEN3_TTS_VERSION` (qwen),
and the per-model spend rates `TTS_COST_PER_1K_CHARS` /
`TTS_COST_PER_1K_CHARS_TURBO` / `TTS_COST_PER_1K_CHARS_QWEN3` (USD per 1,000
input characters; set every rate to 0 to hide the Studio cost readouts).
