...
```
`APP_KEY` is generated by Forge during install. Both Chatterbox models ship as
config defaults with pinned versions (classic `resemble-ai/chatterbox` and
`resemble-ai/chatterbox-turbo`), so you only need the token. To override the
defaults: `REPLICATE_CHATTERBOX_MODEL`/`REPLICATE_CHATTERBOX_VERSION` (classic),
...
`TTS_COST_PER_1K_CHARS_TURBO` (USD per 1,000 input characters; set every rate to
0 to hide the Studio cost readouts).
