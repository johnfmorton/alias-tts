...
Each provider stays **dormant** until its credentials are set: the "Connect" button reads *Not configured* and the OAuth routes refuse with a friendly message instead of erroring. So you can ship without SSO and turn it on later.
...
- **Connect** (signed in): links the provider identity to your account (`connected_accounts` table; unique per provider identity and per user).
