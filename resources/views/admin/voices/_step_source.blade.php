...
<x-voice.step step="source" number="2" title="Voice source" hint="what the voice is made from"
    ...
    <div data-engine-picker class="mt-4 hidden grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div data-engine-only="{{ $presetEngines->implode(' ') }}" @class(['hidden' => ! $presetEngines->contains($engineModel)])>
            ...
            <p data-engine-only="chatterbox-turbo" @class(['mt-2 text-[12.5px] leading-relaxed text-zinc-500', 'hidden' => $engineModel !== 'chatterbox-turbo'])>Turbo needs a reference clip <strong>longer than 5 seconds</strong> — or one of these built-ins instead of a clip.</p>
            <p data-engine-only="qwen3-tts" @class(['mt-2 text-[12.5px] leading-relaxed text-zinc-500', 'hidden' => $engineModel !== 'qwen3-tts'])>Qwen clones from a reference clip of <strong>at least 3 seconds</strong> (aim for 15–20s) — or speaks through one of these built-ins instead.</p>
        </div>
    </div>
