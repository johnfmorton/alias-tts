            ...
            <ul class="divide-y divide-zinc-800">
                ...
                @foreach($projects as $project)
                    <li class="flex items-center justify-between gap-3 px-5 py-3">
                        ...
                        @php
                            // A code glyph shared by the API-origin badges — signals "made
                            // by a programmatic call" without leaning on color alone.
                        ...
                        @if($project->origin === 'api_failure')
                            <span class="inline-flex shrink-0 items-center gap-1 rounded-md border border-red-500/30 bg-red-500/10 px-2 py-0.5 text-xs text-red-300" title="{{ $project->failure_reason }}">{!! $apiGlyph !!}API failure</span>
                        @elseif($project->origin === 'api')
                            <span class="inline-flex shrink-0 items-center gap-1 rounded-md border border-violet-500/30 bg-violet-500/10 px-2 py-0.5 text-xs text-violet-300" title="Generated by a text-to-speech API call (ElevenLabs / OpenAI compatible).">{!! $apiGlyph !!}API</span>
                        @elseif($project->origin === 'api_project')
                            <span class="inline-flex shrink-0 items-center gap-1 rounded-md border border-violet-500/30 bg-violet-500/10 px-2 py-0.5 text-xs text-violet-300" title="Created by a /v1/projects API call — chunked and ready to generate.">{!! $apiGlyph !!}API project</span>
                        @endif
