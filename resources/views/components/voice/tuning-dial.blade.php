@props(['name', 'label', 'hint', 'min', 'max', 'value' => ''])
{{-- A default-tuning dial: a number field and a visual slider that are two views
     ...
     default; the slider then rests at neutral without writing a value. --}}
<div data-tuning-dial>
    ...
    <div class="flex items-center gap-3.5">
        <input id="{{ $name }}" name="{{ $name }}" type="number" step="0.05" min="{{ $min }}" max="{{ $max }}"
               value="{{ $value }}" placeholder="0.5" data-tuning-number
        ...
        <input type="range" min="{{ $min }}" max="{{ $max }}" step="0.05" value="{{ $value !== '' ? $value : 0.5 }}"
